...
当前已完成题目理解、数据结构核对、baseline 搭建、一次 `20 epoch` 实跑验证，以及一轮相关文献初步阅读，项目已进入“结果分析、文献调研与模型优化方案明确”阶段。
...
4. 现阶段尚未加入学习率调度、类别加权、`label smoothing`、注意力模块等优化手段。
5. 当前最佳模型仍以验证集准确率为主要保存依据，实验评价维度还不够完整。

## 下一步计划

1. 使用 `--pretrained` 重新训练 baseline，并把 `macro F1` 与各类别召回率纳入主要比较指标。
2. 增加学习率调度器与 `label smoothing`，降低后期震荡并缓解过度自信预测。
3. 尝试类别加权、`Focal Loss` 或采样策略，重点压低腺癌和鳞癌误诊率。
4. 试验轻量注意力模块，如 `SE`、`CBAM` 或其他全局上下文增强模块。
5. 基于混淆矩阵与高频误判样本开展误差分析，必要时补充 `Grad-CAM` 等可解释性检查。
6. 视运行资源情况决定是否切换 GPU 环境或缩短迭代试验周期。
