param(
    ...
    [int]$BatchSize = 16,
    [int]$ImageSize = 256,
    [string]$ModelName = "efficientnet_b2"
)
...
$weightsRoot = Join-Path $outputsDir "weights"
$resultsRoot = Join-Path $outputsDir "results"
$logDir = Join-Path $outputsDir "cascade_batch_logs"
$null = New-Item -ItemType Directory -Force -Path $logDir
$null = New-Item -ItemType Directory -Force -Path $weightsRoot
$null = New-Item -ItemType Directory -Force -Path $resultsRoot
...
$jobs = @(
    @{ Name = "v2.0_pretrained_ce_b2"; Args = @("--pretrained", "--loss", "cross_entropy") }
    @{ Name = "v2.1_pretrained_ce_cosine_b2"; Args = @("--pretrained", "--loss", "cross_entropy", "--scheduler", "cosine") }
    @{ Name = "v2.2_pretrained_ce_ls_b2"; Args = @("--pretrained", "--loss", "cross_entropy", "--label-smoothing", "0.1") }
    @{ Name = "v2.3_pretrained_ce_ls_cosine_b2"; Args = @("--pretrained", "--loss", "cross_entropy", "--label-smoothing", "0.1", "--scheduler", "cosine") }
    @{ Name = "v2.4_pretrained_ce_ls_cosine_weightedce_b2"; Args = @("--pretrained", "--loss", "cross_entropy", "--label-smoothing", "0.1", "--scheduler", "cosine", "--class-weighting", "balanced") }
    @{ Name = "v3.0_pretrained_focal_ls_cosine_b2"; Args = @("--pretrained", "--loss", "focal", "--label-smoothing", "0.1", "--scheduler", "cosine") }
    @{ Name = "v3.1_pretrained_focal_ls_cosine_mixup_b2"; Args = @("--pretrained", "--loss", "focal", "--label-smoothing", "0.1", "--scheduler", "cosine", "--mixup-alpha", "0.2") }
    @{ Name = "v3.2_pretrained_focal_ls_cosine_cutmix_b2"; Args = @("--pretrained", "--loss", "focal", "--label-smoothing", "0.1", "--scheduler", "cosine", "--cutmix-alpha", "0.5") }
    @{ Name = "v3.3_pretrained_focal_ls_cosine_se_b2"; Args = @("--pretrained", "--loss", "focal", "--label-smoothing", "0.1", "--scheduler", "cosine", "--feature-attention", "se") }
    @{ Name = "v3.4_pretrained_focal_ls_cosine_cbam_b2"; Args = @("--pretrained", "--loss", "focal", "--label-smoothing", "0.1", "--scheduler", "cosine", "--feature-attention", "cbam") }
)
...
$logPath = Join-Path $logDir "run_all_efficientnet_b2_50_$timestamp.log"
...
try {
    ...
    foreach ($job in $jobs) {
        ...
        $mainCkpt = Join-Path $weightsRoot "$runName\best_model.pt"
        $mainSummary = Join-Path $resultsRoot "$runName\metrics_summary.json"
        ...
        $tumor3ExpertCkpt = Join-Path $weightsRoot ("expert_tumor3_" + $runName + "\best_model.pt")
        $tumor3CascadeOut = Join-Path $outputsDir ("cascade_" + $runName)
        $tumor3CascadeSummary = Join-Path $resultsRoot ("cascade_" + $runName + "\metrics_summary.json")
        ...
        foreach ($pair in $expertPairs) {
            ...
            $pairExpertCkpt = Join-Path $weightsRoot ("expert_pair_" + $pairTag + "_" + $runName + "\best_model.pt")
            $pairCascadeOut = Join-Path $outputsDir ("cascade_pair_" + $pairTag + "_" + $runName)
            $pairCascadeSummary = Join-Path $resultsRoot ("cascade_pair_" + $pairTag + "_" + $runName + "\metrics_summary.json")
    ...
    Write-Host ""
    Write-Host "All EfficientNet-B2 formal experiments finished."
