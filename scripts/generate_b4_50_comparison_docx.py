...
DOC_DATE = "20260721"
DOC_NAME = f"b4_50_experiments_comparison_{DOC_DATE}.docx"
RUN_SUFFIX = "_b4"
...
def family_order() -> list[str]:
    return [
        "v2.0_pretrained_ce_b4",
        "v2.1_pretrained_ce_cosine_b4",
        "v2.2_pretrained_ce_ls_b4",
        "v2.3_pretrained_ce_ls_cosine_b4",
        "v2.4_pretrained_ce_ls_cosine_weightedce_b4",
        "v3.0_pretrained_focal_ls_cosine_b4",
        "v3.1_pretrained_focal_ls_cosine_mixup_b4",
        "v3.2_pretrained_focal_ls_cosine_cutmix_b4",
        "v3.3_pretrained_focal_ls_cosine_se_b4",
        "v3.4_pretrained_focal_ls_cosine_cbam_b4",
    ]
...
def load_formal_results(results_root: Path) -> list[RunResult]:
    ...
    if missing or extra:
        messages: list[str] = []
        if missing:
            messages.append(f"缺少 {len(missing)} 个正式实验结果：{missing}")
        if extra:
            messages.append(f"发现 {len(extra)} 个未识别结果：{extra}")
...
def build_document_xml(results: list[RunResult]) -> str:
    ...
    body: list[str] = []
    body.append(make_paragraph("EfficientNet-B4 50组正式实验结果对比", bold=True, size=32, align="center"))
    body.append(
        make_paragraph(
